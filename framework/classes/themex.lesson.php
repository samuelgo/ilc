		...
		if(self::$data['progress']==0) {
			...
			if(!empty(self::$data['quiz']['questions'])) {
				...
				if(self::$data['quiz']['percentage']>0 && $percentage>=self::$data['quiz']['percentage']) {
					...
					ThemexInterface::$messages[]=sprintf(__('You require %d%% to pass this quiz.', 'academy'), self::$data['quiz']['percentage']);
				}
			}
		} else {
		...
		if(get_post_type()=='lesson') {
			...
			if($comment['comment_parent']!==0 && !empty($message)) {
				...
				if(in_array($comment['comment_author_email'], $emails)) {
					unset($emails[array_search($comment['comment_author_email'], $emails)]);
				}
