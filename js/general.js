...
jQuery(document).ready(function($) {
	//Código Concepto Web Javascript 1.1
	var $modulos = $('#modulos');
	var $preguntas = $('.questions li a');
	...
	/* Código Concepto Web Javascript 1.2 */

	/*Muestra las lecciones (módulos) al hacer scroll */
	$(window).on('scroll', function() {
			if($(window).scrollTop() +1 >= ($(document).height() - $(window).height() ) / 2 && $('.public-modules').length == 0 ) {
				$('#modulos > .row:hidden').each(function(i) { // A cada módulo oculto
					var obj = $(this);
					obj.eq(i).delay(150).fadeIn(1000);
				});
			}
	});

	/*Scrolling directorio*/
	$(window).on('scroll', function() {
		$('#directorio  > .row:hidden').each(function(i) { // A cada usuario oculto en el directorio
			var obj = $(this);
			obj.eq(i).delay(150).fadeIn(1000);
		});
	});

	/*Popup para los módulos*/
	if($modulos.find('.popup').length) { // Si .popup existe...
		// ... al hacer clic en las preguntas
		$preguntas.on('click', function(e) {
			e.preventDefault();
			$modulos.find('.popup').fadeIn(500).delay(500).fadeOut(500); // Mostrará el mensaje emergente.
		});
	}
...
var username = jQuery("#st-username").val();
 ...
 var certificado = jQuery("#Certificado").val();
...
$('#Certificado').on('change', function(e) {
	...
	$.ajax({
		type:'POST',
		url: 'http://127.0.0.1/academy/wp-content/themes/academy/custom/user-check.php',
		data: 'certificado='+ $('#Certificado').val(),
